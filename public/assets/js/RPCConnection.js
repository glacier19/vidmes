...
function USRPCConnection( systemoption )
{
	...
	this.last_time 			= "";
	this.maxUploadSize 		= parseInt(this.systemoption.maxuploadsize);
	this.sdpDescription;			
	...
	this.disconnect             = USRPCConnection_disconnect;
...
{	
	...
	if (RPC.creator)
	{	// Yes, it is creator
		console.log("_createRoom", "creator", RPC.creator);
		// Display room in 5 seconds
		setTimeout(RPC._createRoom, RPC.createRoomTimeout);		
	}
	else
	{	// No, it is opponent
		console.log("reciveConnectMessage", "creator", RPC.creator);
...
function USRPCConnection_handleFileUpload(files,dropZone) 
{
	...
	else 
	{
		...
		formData.append("creator", RPC.creator);
...
function USRPCConnection_cb_gotRemoteStream()
{
...
// *******************
function USRPCConnection_disconnect() 
{
  var RPC = window.USRPCConnection;
  //if(ev.target.iceConnectionState == 'disconnected') 
  //{
